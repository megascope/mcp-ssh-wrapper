...
`mcp-ssh-server` is a minimal MCP server that executes commands on remote hosts by delegating to the local `ssh` binary.
...
```bash
mcp-ssh-server --host 127.0.0.1 --port 8000 --mount-path /mcp
```
...
```bash
python -m mcp_ssh_server --host 127.0.0.1 --port 8000 --mount-path /mcp
```
...
`run_test.sh` does not launch the server. Start `mcp-ssh-server ...` or `./run_server.sh ...` first, then run the test wrapper against the live endpoint.
